...
local nos__zhugezhan = General(extension, "nos__zhugezhan", "shu", 3)
local nos__zuilun = fk.CreateActiveSkill{
  name = "nos__zuilun",
  anim_type = "control",
  prompt = "#nos__zuilun",
  card_num = 0,
  ...
  on_use = function(self, room, effect)
    ...
    local card = room:askForCardChosen(player, target, flag, self.name, "#nos__zuilun-prey::"..target.id)
...
local nos__fuyin = fk.CreateProhibitSkill{
  ...
  is_prohibited = function(self, from, to, card)
    return to:hasSkill(self) and #to:getEquipments(Card.SubtypeArmor) == 0 and
      from ~= to and from:getHandcardNum() >= to:getHandcardNum() and
      card and table.contains({"slash", "duel", "fire_attack"}, card.trueName)
  end,
}
nos__zhugezhan:addSkill(nos__zuilun)
nos__zhugezhan:addSkill(nos__fuyin)
Fk:loadTranslationTable{
  ["nos__zhugezhan"] = "诸葛瞻",
  ["#nos__zhugezhan"] = "临难死义",
